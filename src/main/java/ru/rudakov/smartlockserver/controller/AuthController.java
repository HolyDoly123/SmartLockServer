package ru.rudakov.smartlockserver.controller;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            Role userRole = roleRepository.findByName(RoleEnum.ROLE_GUEST)
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin":
                        ...
                        break;
                    case "guest":
                        Role modRole = roleRepository.findByName(RoleEnum.ROLE_GUEST)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(modRole);
                        break;
                    default:
                        Role userRole = roleRepository.findByName(RoleEnum.ROLE_FAMILY_MEMBER)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(userRole);
                }
            });
